...
n_episodes = 100001 #1001 # n games we want agent to play (default 1001) the more games we play the more data we get for training
...
class DQNAgent:
    def __init__(self, state_size, action_size):
        ...
        self.tensorboard = ModifiedTensorBoard(log_dir="logs\{}-{}".format(MODEL_NAME, int(datetime.datetime.now().strftime("%f"))))#log_dir="logs\{}-{}".format(MODEL_NAME, int(Z)))#(log_dir=f"logs/{MODEL_NAME}-{int(time.time())}")
...
for e in tqdm(range(1, n_episodes + 1 ), ascii=True, unit='eposodes'): 
    ...
    #reset flag and start iterationg until episode ends
    done = False #default is game has not ended.
    while not done:
        if np.random.random() > epsilon_x:
        ...
        #transform new continous stae to new discrete state and count reward
        episode_reward += reward

        #every step we update replay memory and train main network
                       #current_state, state, action, reward, next_state, done
    ...
    if not e % AGGREGATE_STATS_EVERY or e == 1:
        ...
        if min_reward >= MIN_REWARD: #>= MIN_REWARD:
            agent.model.save(f'models\{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(datetime.datetime.now().strftime("%f"))}.model')
    ...
    #     #tensorboard --port=6007 --logdir=C:\Users\cgree\Documents\Astra\Space_weather5_22\weakley_all\models
    #     #tensorboard --port=6006 --logdir=C:\Users\cgree\Documents\Astra\Space_weather5_22\weakley_all\logs
