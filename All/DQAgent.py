...
#         env.render() #moving my animation to render
    ...
    if not e % AGGREGATE_STATS_EVERY or e == 1:
        ...
        if min_reward >= MIN_REWARD: #>= MIN_REWARD:
            agent.model.save(f'X:\Final_RL\models\{MODEL_NAME}__{max_reward:_>7.2f}max_{average_reward:_>7.2f}avg_{min_reward:_>7.2f}min__{int(datetime.datetime.now().strftime("%f"))}.model')
