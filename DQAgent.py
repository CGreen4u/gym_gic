...
state_size = 1 #env.observation_space.n #we changed this to one because ther is only one input for state as of now
...
n_episodes = 2001 # n games we want agent to play (default 1001) the more games we play the more data we get for training
...
class DQNAgent:
    def __init__(self, state_size, action_size):
        ...
        self.memory = deque(maxlen=20000) # double-ended queue; acts like list, but elements can be added/removed from either end 
...
#         env.render() #moving my animation to render
        ...
        next_state, reward, done, _ = env.step(np.array(action)) #env.step(np.array([guess])) # agent interacts with env, gets feedback; 4 state data points, e.g., high or low - observation - later the variables and  done, reward        
        print(reward)
